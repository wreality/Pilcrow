 ...
 *
 * Value names use {@see AbilityExposure::wireName} (`Str::snake` of the case
...
GRAPHQL;
    ...
    public function manipulateTypeDefinition(DocumentAST &$documentAST, TypeDefinitionNode &$typeDefinition): void
    {
        ...
        foreach ($exposed as $wireName => $exposure) {
            if (isset($existing[$wireName])) {
                continue;
            }
            $description = str_replace('"""', '', $exposure['description']);
            $typeDefinition->values[] = Parser::enumValueDefinition(
                "\"\"\"{$description}\"\"\"\n{$wireName}"
            );
        }
    }
