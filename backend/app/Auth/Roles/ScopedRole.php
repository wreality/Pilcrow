...
enum ScopedRole: string
{
     ...
     *
     * This is the corrected submission ability matrix: the intent abilities
     * ({@see SubmissionAbility::Review}, {@see SubmissionAbility::Submit}) and the
     * re-seated {@see SubmissionAbility::UpdateContent} (author/draft body + file +
     * title, folding in the old UpdateTitle — closes the manuscript-edit hole) are
     * in place, and the deprecated `updateSubmission` god-mutation and its
     * LegacyUpdate bridge are gone. A few refinements still remain as standalone
     * follow-ups (drop Reviewer `View`, split `UpdateSubmitters` into
     * draft↔not-draft, retire the submitter's draft-only `UpdateStatus`).
     *
    ...
    private function grantDefinitions(): array
    {
        return match ($this) {
            self::Reviewer => [
                ...
                // while under review. (A later refinement drops View here; kept for
                // now so reviewer item-reads survive until the review surface gates
                // on `review`.)
            ...
            self::Submitter => [
                ...
                SubmissionAbility::UpdateSubmitters,
                // A submitter may still set status while the work is theirs (DRAFT);
                // submitSubmission (Submit) is the dedicated forward action, but
                // changeSubmissionStatus stays open to them for a draft. Candidate
                // to retire in the matrix refinement now that submit is its own gate.
