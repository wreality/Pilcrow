...
class AbilitiesTest extends ApiTestCase
{
     ...
     * exposed wire names — if the directive breaks, an enum and its GraphQL
    ...
    public function testAbilityEnumValuesAreGeneratedFromExposedCases(string $typeName, string $enum): void
    {
        ...
        foreach ($exposed as $wireName => $exposure) {
            $this->assertSame($exposure['description'], $values[$wireName]['description']);
        }
    }
    ...
    public function testEveryExposedScopedAbilityIsGrantedSomewhere(): void
    {
        ...
        foreach ([SubmissionAbility::class, PublicationAbility::class] as $enum) {
            foreach (AbilityExposure::exposed($enum) as $wireName => $exposure) {
                $ability = $exposure['case'];
                $this->assertArrayHasKey(
                    $ability::class . '::' . $ability->name,
                    $grantedAbilities,
                    "Exposed {$ability->name} ({$wireName}) is granted by no role."
                );
            }
        }
    }
