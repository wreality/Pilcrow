...
class SubmissionTest extends ApiTestCase
{
    ...
    public function testUpdatedByFieldIsSet()
    {
        ...
        $response = $this->graphQL(
            'mutation UpdateSubmissionContent($id: ID!, $title: String) {
                updateSubmissionContent(input: {
                    id: $id
                    title: $title
                }) {
                    created_by {
        ...
        $response->assertJsonPath('data.updateSubmissionContent.created_by.id', (string)$submission->created_by);
        $response->assertJsonPath('data.updateSubmissionContent.updated_by.id', (string)$user->id);
    }
